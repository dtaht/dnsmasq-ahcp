...
pulling some needed ideas from ahcp.c
...
void ahcp_init(void)
{
  ...
  if ((fd = socket(PF_INET6, SOCK_DGRAM, IPPROTO_UDP)) == -1 ||
      ...
      !fix_fd(fd) ||
      !set_ipv6pktinfo(fd))
    die (_("cannot create AHCP socket: %s"), NULL, EC_BADNET);
  
  /* When bind-interfaces is set, there might be more than one dnmsasq
     instance binding the port. That's OK if they serve different networks.
...
#endif
      if (rc == -1)
	die(_("failed to set SO_REUSE{ADDR|PORT} on AHCP socket: %s"), NULL, EC_BADNET);
    }
  ...
  saddr.sin6_port = htons(AHCP_SERVER_PORT);
  
  if (bind(fd, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in6)))
    die(_("failed to bind AHCP server socket: %s"), NULL, EC_BADNET);
...
void ahcp_packet(time_t now)
{
  ...
  msg.msg_iov =  &daemon->ahcp_packet;
  msg.msg_iovlen = 1;
  
  if ((sz = recv_dhcp_packet(daemon->ahcpfd, &msg)) == -1)
    return;
  ...
  if (!indextoname(daemon->ahcpfd, if_index, ifr.ifr_name))
    return;
  ...
  for (context = daemon->ahcp; context; context = context->next)
  ...
  port = ahcp_reply(parm.current, if_index, ifr.ifr_name, &parm.fallback, 
  ...
  if (port != 0)
    {
      ...
      while (sendto(daemon->ahcpfd, daemon->outpacket.iov_base, save_counter(0), 
...
/* end of currently commented out code */
#endif
